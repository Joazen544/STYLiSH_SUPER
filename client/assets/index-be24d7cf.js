...
`;function Gv(){const[e,t]=C.useState([]),[n,r]=C.useState(0),i=C.useRef();return C.useEffect(()=>{async function o(){const{data:l}=await mt.getCampaigns();console.log(l),t(l),i.current=window.setInterval(()=>{r(a=>a===l.length-1?0:a+1)},5e3)}o()},[]),c.jsxs(_v,{children:[e.map(({picture:o,product_id:l,story:a},s)=>c.jsx($v,{$isActive:s===n,$backgroundImageUrl:o,to:`/products/${l}`,children:c.jsxs(Vv,{children:[c.jsx(Wv,{children:a.split(`\r
`).slice(0,3).join(`\r
...
`;function by({product:e}){const[t,n]=C.useState(),[r,i]=C.useState(),[o,l]=C.useState(0),{cartItems:a,setCartItems:s}=C.useContext(si);function u(h,y){var k;if(console.log("colorCode in getStock",h),console.log("size in getStock",y),!h||!y)return console.log("nonononono"),0;const x=((k=a.find(R=>R.id===e.id&&R.color.code===h&&R.size===y))==null?void 0:k.qty)||0;return e.variants.find(R=>R.color_code===h&&R.size===y)?e.variants.find(R=>R.color_code===h&&R.size===y).stock-x:0}function f(){if(!t){window.alert("請選擇顏色");return}if(!r){window.alert("請選擇尺寸");return}if(o===0){window.alert("請選擇數量");return}const h=a.findIndex(x=>x.id===e.id&&x.color.code===t&&x.size===r),y=h!==-1?a.map((x,k)=>k===h?{...x,qty:x.qty+o}:x):[...a,{color:e.colors.find(x=>x.code===t),id:e.id,image:e.main_image,name:e.title,price:e.price,qty:o,size:r,stock:u(t,r)}];s(y),n(),i(),l(0),window.alert("已加入商品")}function m(){return t?r?o===0?"請選擇數量":"加入購物車":"請選擇尺寸":"請選擇顏色"}return c.jsxs(c.Fragment,{children:[c.jsxs(bl,{children:[c.jsx(Il,{children:"顏色｜"}),e.colors.map(h=>c.jsx(Ey,{$isSelected:h.code===t,$colorCode:`${h.code}`,onClick:()=>{n(h.code),i(null),l(0)}},h.code))]}),c.jsxs(bl,{children:[c.jsx(Il,{children:"尺寸｜"}),e.sizes.map(h=>{const y=u(t,h);return console.log("stock does have value",y),c.jsx(jy,{$isSelected:h===r,$isDisabled:y===0,onClick:()=>{y!==0&&(i(h),l(0))},children:h},h)})]}),c.jsxs(bl,{children:[c.jsx(Il,{$hideonmobile:!0,children:"數量｜"}),c.jsxs(Py,{children:[c.jsx(Ny,{onClick:()=>{!r||o===0||l(o-1)}}),c.jsx(Ry,{children:o}),c.jsx(Ty,{onClick:()=>{const h=u(t,r);!r||o===h||l(o+1)}})]})]}),c.jsx(zy,{onClick:f,children:m()})]})}function Iy(){const[e,t]=C.useState(),[n,r]=C.useState([]),{id:i}=Vm(),o=C.useRef();return C.useEffect(()=>{async function l(){const{data:s}=await mt.getProduct(i);t(s[0])}async function a(){const s=await mt.getRecommendation();r(s.data)}a(),l()},[i]),e?c.jsxs(My,{children:[c.jsx(Oy,{src:e.main_image}),c.jsxs(Ly,{children:[c.jsx(Fy,{children:e.title}),c.jsx(Dy,{children:e.id}),c.jsxs(By,{children:["TWD.",e.price]}),c.jsx(by,{product:e}),c.jsx(Uy,{children:e.note}),c.jsx(_y,{children:e.texture}),c.jsx(Up,{children:e.description}),c.jsxs(Oc,{children:["素材產地 / ",e.place]}),c.jsxs(Oc,{children:["加工產地 / ",e.place]})]}),c.jsxs(Op,{isProductPage:!0,children:[c.jsx(Eo,{position:"left",onMoveToPrev:()=>Ao.moveToPreviousSlide(o)}),c.jsx(Fp,{text:"大家都在買"}),c.jsx(Lp,{ref:o,children:n.map(({main_image:l,colors:a,title:s,id:u},f)=>c.jsx(Dp,{image:l,colors:a,title:s,id:u},f))}),c.jsx(Eo,{position:"right",onMoveToNext:()=>Ao.moveToNextSlide(o)})]}),c.jsxs($y,{children:[c.jsx(Vy,{children:"細部說明"}),c.jsx(Wy,{children:e.story})]}),c.jsx(Hy,{children:e.images.map((l,a)=>c.jsx(Qy,{src:l},a))})]}):null}const My=E.div`
  max-width: 960px;
  margin: 0 auto;
  padding: 65px 0 49px;
  display: flex;
